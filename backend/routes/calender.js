...
router.post('/exercisevideo/:id/:routineid',videoUpload.single('video'),async (req, res) => {
    if (req.session.loggedin) {
      try {
        ...
        if (!exerciseRoutine) {
          return res.status(400).json({ data: null, message: '해당 운동 루틴이 존재하지 않습니다.' });
        }
        ...
        res.status(200).json({ data: null, message: '운동영상 업로드가 완료되었습니다.' });
      } catch (err) {
        console.log(err);
