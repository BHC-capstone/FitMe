...
router.post("/ptrequest", (req, res) => {
    pt_requests
        .findOne({
            where: { trainer_id: req.body.trainerid, user_id: req.body.userid },
        })
        .then((requestInfo) => {
            ...
            if (requestInfo == undefined) {
                ...
                pt_requests.create(newRequest).then(() => {
                    res.status(200).json({
                        data: null,
                        message: "성공적으 로 신청되었습니다.",
                    });
                });
            } else {
                res.status(401).json({
                    data: null,
                    message: "이미 신청한 trainer입니다",
                });
            }
        });
});
                    ...
                    .then(() => {
                        ...
                        trainer_manage
                            ...
                            .catch((error) => {
                                res.status(500).json({
                                    data: null,
                                    message: "trainerManage insert 오류",
                                });
                            });
                    })
                    .catch((error) => {
